...
function RegisterStudent(){ 
    ...
    studentData['average'] = calcAverage(studentData['mathMarks'],studentData['engMarks']);
...
function displayData(){
    for(var index = 1 ; index <= sessionStorage.getItem('counter') ; index ++ ){
        document.querySelector('#displayData').innerHTML += `<tr><td>${index}</td><td>${sessionStorage.getItem(`StudentName${index}`)}</td></td><td>${sessionStorage.getItem(`StudentMathMark${index}`)}</td><td>${sessionStorage.getItem(`StudentEngMark${index}`)}</td><td>${sessionStorage.getItem(`StudentAvg${index}`)}</td><td>${sessionStorage.getItem(`StudentYear${index}`)}</td><td>${sessionStorage.getItem(`CreatedDate${index}`)}</td></tr>`; 
    }
}
